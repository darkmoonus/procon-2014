...
public class ColorFormulas {
	...
	public void RGBtoXYZ(int RVal, int GVal, int BVal) {
		...
		double R = (double) (RVal) / 255.0; // R from 0 to 255
		double G = (double) (GVal) / 255.0; // G from 0 to 255
		double B = (double) (BVal) / 255.0; // B from 0 to 255
	...
	public int CompareTo(ColorFormulas oComparisionColor) {
		double DeltaE = Math.sqrt(Math.pow((CieL - oComparisionColor.CieL), 2)
				+ Math.pow((CieA - oComparisionColor.CieA), 2)
				+ Math.pow((CieB - oComparisionColor.CieB), 2));
		// return Convert.ToInt16(Math.round(DeltaE));
		return (int) (Math.round(DeltaE));
	}
